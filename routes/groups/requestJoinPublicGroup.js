...
module.exports = async (req, res) => {
    ...
    try {
      ...
      const checkIfGroupIsPrivateQuery = `SELECT is_private	FROM collegepickems."PickemGroups" WHERE id = $1;`
      ...
      if (checkIfPublicGroupResult.rows[0].is_private === true) {
        ...
        const checkIfRequestAlreadyExistsQuery = `SELECT COUNT(*)	FROM collegepickems."PickemGroupMembers" WHERE user_id = $1 AND pickem_group_id = $2;`
        ...
        if (checkIfRequestAlreadyExistsResult.rows[0].count === '1') {
          ...
          const insertQuery = `
          INSERT INTO collegepickems."PickemGroupMembers" (user_id, pickem_group_id, is_admin, is_active, pending_activation)
